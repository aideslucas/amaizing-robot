...
float Particle::ProbByLaser(float laserArray[]) {
	...
	// Run over laser scan
	for (unsigned int laserRead = 0; laserRead < 666;
			laserRead += 10) {
		totalReadingAmount++;
...
int Particle::checkIfLaserDetectRight(float laserScanValue, int laserRead) {
	...
	for (int distanceFromSpottedPointInCM = 1;
			...
			distanceFromSpottedPointInCM += 15) {
		...
		Point spottedPoint;
		spottedPoint.x = round(_position.getCol() + deltaCol);
		spottedPoint.y = round(_position.getRow() + deltaRow);

		// check if the spotted point is in the map
		if ((spottedPoint.x >= 0
				&& spottedPoint.x < _map->getWidth())
				&& (spottedPoint.y >= 0
						&& spottedPoint.y < _map->getHeight())) {

			// get the value of point from map
			bool spottedPointValue =
					_map->checkIfCellIsOccupied(spottedPoint);

			// check if the spotted point is block and we are NEAR the laserScanValueInCM
			if (spottedPointValue) {
				if (abs(
						laserScanValueInCM
